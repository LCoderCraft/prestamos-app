...
<body class="bg-gradient-to-br from-indigo-100 via-gray-50 to-indigo-100 min-h-screen">
    ...
    <div class="max-w-6xl mx-auto p-4 mt-6">
        ...
        <div class="mb-4 border-b border-gray-200">
            <nav class="flex -mb-px space-x-6">
                ...
                <button onclick="switchTab('history')" id="tab-history" class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm text-gray-500 border-transparent hover:text-gray-700">
                    Historial Completo
                </button>
                <button onclick="switchTab('inventory')" id="tab-inventory" class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm text-gray-500 border-transparent hover:text-gray-700">
                    Inventario
                </button>
            </nav>
        </div>
        ...
        <div class="bg-white p-8 rounded-lg shadow-xl w-full">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm flex space-x-2">
                                    @if($loan->status == 'pending')
                                        ...
                                        <button onclick="confirmReject({{ $loan->id }})" class="text-xs bg-red-500 text-white px-2 py-1 rounded">✖ Rechazar</button>
            ...
            <div id="content-history" class="hidden">
                <h2 class="text-2xl font-bold text-gray-700 mb-6">Historial de Devoluciones y Rechazos</h2>
                <div class="overflow-x-auto">
                    <table class="w-full min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Usuario</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Estado</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Observaciones / Motivo</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm">
                                    @if($loan->status == 'finished')
                                        <span class="px-2 py-1 text-xs font-bold rounded-full bg-gray-200 text-gray-800">Finalizado</span>
                                    @else
                                        <span class="px-2 py-1 text-xs font-bold rounded-full bg-red-100 text-red-800">Rechazado</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 italic border-l-4 border-indigo-100 bg-gray-50">
                                    {{ str_replace('DEVOLUCIÓN: ', '', $loan->admin_comment) }}
                                </td>
                            </tr>
                            @empty
                            <tr><td colspan="5" class="p-4 text-center text-gray-500">El historial está vacío.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            ...
            <div id="content-inventory" class="hidden">
                ...
                <div class="bg-indigo-50 p-4 rounded-lg mb-8 border border-indigo-100">
                    ...
                    <form action="{{ route('admin.items.store') }}" method="POST" class="flex flex-col md:flex-row gap-4 items-end">
                        @csrf
                        <div class="flex-1 w-full"><label class="text-xs text-gray-600 font-bold">Nombre</label><input type="text" name="name" class="w-full p-2 border rounded text-sm" required></div>
                        <div class="w-24"><label class="text-xs text-gray-600 font-bold">Cant.</label><input type="number" name="total_count" value="1" min="1" class="w-full p-2 border rounded text-sm" required></div>
                        <div class="flex-1 w-full"><label class="text-xs text-gray-600 font-bold">URL Foto</label><input type="url" name="photo_url" class="w-full p-2 border rounded text-sm"></div>
    ...
    <div id="return-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 flex backdrop-blur-sm">
        <div class="bg-white p-6 rounded-lg shadow-2xl w-96">
            <h3 class="text-xl font-bold mb-4 text-indigo-700 border-b pb-2">Finalizar Préstamo</h3>
            <p class="text-sm text-gray-600 mb-4">Usuario: <span id="return-user" class="font-bold"></span></p>
            <p class="text-sm text-gray-600 mb-4">Producto: <span id="return-item" class="font-bold"></span></p>
            <form id="return-form" method="POST">
                @csrf <input type="hidden" name="action" value="finish">
                <label class="block text-sm font-bold mb-2 text-gray-700">Estado del Material / Observaciones:</label>
                <textarea name="comment" rows="3" class="w-full border p-2 rounded text-sm" placeholder="Ej: Entregado correctamente..."></textarea>
    ...
    <script>
        ...
        function openReturnModal(id, user, item) {
            ...
            document.getElementById('return-item').textContent = item;
            let url = "{{ route('admin.loans.update', ':id') }}";
        ...
        function openEditModal(item) {
            ...
            let url = "{{ route('admin.items.update', ':id') }}";
